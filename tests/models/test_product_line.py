...
def test_model_structure_column_constraints(db_inspector):
    ...
    assert any(constraint["name"] == "product_line_check_price" for constraint in constraints)
...
def test_model_structure_unique_constraints(db_inspector):
    ...
    assert any(constraint["name"] == "uq_product_line_sku" for constraint in constraints)
    assert any(constraint["name"] == "uq_product_line_order_product_id" for constraint in constraints)
...
def test_model_structure_foreign_key(db_inspector):
    ...
    product_foreign_key = next((fk for fk in foreign_keys if fk["constrained_columns"] == ["product_id"]), None)
